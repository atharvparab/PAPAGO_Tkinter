    ...
    while found == 0:
     ...
     """)
        cursor.execute(insertData,
                       [(name_var_register), (username_var_register), (password_var_register), (contact_var_register),
                        (email_var_register)])
        db.commit()
...
def travel_table():
    ...
    my_tree = ttk.Treeview(treeview, columns=(
...
def payment_table():
    ...
    db.commit()
    treeview = tk.Toplevel()
    my_tree = ttk.Treeview(treeview, columns=('Payment ID', 'User ID', 'Payment Mode', 'Total Amount'), show="headings",
                           height="5")
    my_tree.pack(fill="both", expand=True)
...
login_button = tk.Button(frame2, text="LOGIN ", font='Times 20', bg="#AA2B1D",
                         command=lambda: [validatefieldslogin(), check_credentialslogin()])
...
# button
bt = tk.Button(frame10, text="CONTINUE", bg="#AA2B1D", font="Times 20",
               command=lambda: [traveldetails(), get_booking()])
...
bt = tk.Button(frame12, text="CONTINUE", bg="#AA2B1D", font="Times 20",
               command=lambda: [get_payment(), get_total_amount()])
...
mybutton = tk.Button(frame14, bg="#AA2B1D", text="Get Receipt ", font='Times 25',
                     command=lambda: [grab_date_receipt(), get_receipt(), print_receipt()])
...
btn = tk.Button(frame16, text="PRINT RECEIPT", bg="#AA2B1D", font="Times 20 italic",
                command=lambda: [automated_email(), get_logout()])
...
def traveldetails():
    ...
    while (found == 0):
        ...
        with sqlite3.connect('Miniproject.db') as db:
            ...
            cursor = db.cursor()
        insert_details = (
            "INSERT INTO TravelDetails(userID,desID,desNAME,totaladult,totalchild,totalinfant,traveldate) VALUES (?,?,?,?,?,?,?)")
        cursor.execute(insert_details,
                       [(userid_pass), (desid_pass), (destination_var), (adult_var), (child_var), (infant_var),
                        (travel_date)])
        db.commit()
        found = 1
        db.close()
